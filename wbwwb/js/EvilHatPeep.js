...
function EvilHatPeep(scene){
    ...
    self.itsMurderTime = function(){
        ...
        self.setTimeout(function(){
            if(self.victim) self.victim.smile();
        },_s(0.5));
        ...
        self.setTimeout(function(){
            self.hasGunOut = true;
            if(Game.sounds.bg_park) Game.sounds.bg_park.stop(); // STAHP.
            if(Game.sounds.gun_cock) Game.sounds.gun_cock.play(); // SOUND
            MODE = MODE_GUN;
        },_s(0.5+1.5));
        self.setTimeout(function(){
            if(self.freezeEveryone) self.freezeEveryone();
            if(self.victim) self.victim.frown();
        },_s(0.5+1.5+0.5));

        // +4 beats.
        // BANG.
        self.setTimeout(function(){
            if(Game.sounds.gunshot) Game.sounds.gunshot.play();
            if(self.bang) self.bang();
        },_s(0.5+1.5+4.0));
