...
function Stage_Evil(self, HACK) {
    ...
    var _freezeEveryone = function () {
        ...
        if (lovers) lovers.beStunned();

        // GOODBYE PARK SOUND
        if (Game.sounds.bg_park) Game.sounds.bg_park.stop();
        ...
        takePhoto: function (d) {
            ...
            d.tryChyron(function (d) {
                ...
                if (murderer.hasGunOut) {
                    ...
                    if (caught.evil) {
                        d.chyron = "क्या टोपी पहनना अब सामाजिक भूल है?";
                        return true;
                    }
                    return false;
                }
...
function Stage_Panic(self) {
    ...
    self.director.callbacks = {
        takePhoto: function (d) {
            var p = d.photoData;
            d.chyron = "डरिए... क्योंकि मामला गंभीर है!";
            p.forceChyron = true;
